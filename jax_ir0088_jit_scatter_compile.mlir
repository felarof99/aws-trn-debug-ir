...
#loc5 = loc("/home/ubuntu/roadrunner-fork/src/felafax/trainer_engine/checkpoint.py":298:0)
...
module @jit_scatter attributes {mhlo.num_partitions = 2 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<16x512x2048xbf16> {mhlo.sharding = "{devices=[1,2,1]<=[2]}"} loc("args[0]"), %arg1: tensor<1xi32> loc("args[1]"), %arg2: tensor<512x2048xbf16> {mhlo.sharding = "{devices=[2,1]<=[2]}"} loc("args[2]")) -> (tensor<16x512x2048xbf16> {jax.result_info = ""}) {
    ...
    }) : (tensor<16x512x2048xbf16>, tensor<1xi32>, tensor<512x2048xbf16>) -> tensor<16x512x2048xbf16> loc(#loc15)
