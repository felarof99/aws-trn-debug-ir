...
#loc5 = loc("/home/ubuntu/roadrunner-fork/src/felafax/trainer_engine/models/llama3/jax/model.py":317:0)
...
module @jit__normal attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<16x4xui32> loc(unknown)) -> tensor<16x512x2048xbf16> {
    ...
    %1 = call @_normal_real(%0) : (tensor<16x4xui32>) -> tensor<16x512x2048xbf16> loc(#loc22)
    return %1 : tensor<16x512x2048xbf16> loc(#loc)
  } loc(#loc)
  func.func private @_normal_real(%arg0: tensor<16x4xui32> loc("jit(_normal)/jit(main)/pjit"(#loc21))) -> tensor<16x512x2048xbf16> {
    ...
    %0 = call @_uniform(%arg0, %cst, %cst_0) : (tensor<16x4xui32>, tensor<bf16>, tensor<bf16>) -> tensor<16x512x2048xbf16> loc(#loc23)
    %1 = chlo.erf_inv %0 : tensor<16x512x2048xbf16> -> tensor<16x512x2048xbf16> loc(#loc24)
    %cst_1 = stablehlo.constant dense<1.414060e+00> : tensor<bf16> loc(#loc22)
    %2 = stablehlo.broadcast_in_dim %cst_1, dims = [] : (tensor<bf16>) -> tensor<16x512x2048xbf16> loc(#loc25)
    %3 = stablehlo.multiply %2, %1 : tensor<16x512x2048xbf16> loc(#loc25)
    return %3 : tensor<16x512x2048xbf16> loc(#loc22)
  } loc(#loc22)
  func.func private @_uniform(%arg0: tensor<16x4xui32> loc("jit(_normal)/jit(main)/jit(_normal_real)/pjit"(#loc21)), %arg1: tensor<bf16> loc("jit(_normal)/jit(main)/jit(_normal_real)/pjit"(#loc21)), %arg2: tensor<bf16> loc("jit(_normal)/jit(main)/jit(_normal_real)/pjit"(#loc21))) -> tensor<16x512x2048xbf16> {
    ...
    %output_state, %output = stablehlo.rng_bit_generator %5, algorithm =  DEFAULT : (tensor<2xui64>) -> (tensor<2xui64>, tensor<16x512x2048xui8>) loc(#loc29)
    ...
    %10 = stablehlo.convert %output : (tensor<16x512x2048xui8>) -> tensor<16x512x2048xui16> loc(#loc32)
    %c_1 = stablehlo.constant dense<1> : tensor<ui16> loc(#loc23)
    %11 = stablehlo.broadcast_in_dim %c_1, dims = [] : (tensor<ui16>) -> tensor<16x512x2048xui16> loc(#loc33)
    %12 = stablehlo.shift_right_logical %10, %11 : tensor<16x512x2048xui16> loc(#loc33)
    %c_2 = stablehlo.constant dense<16256> : tensor<ui16> loc(#loc23)
    %13 = stablehlo.broadcast_in_dim %c_2, dims = [] : (tensor<ui16>) -> tensor<16x512x2048xui16> loc(#loc34)
    %14 = stablehlo.or %12, %13 : tensor<16x512x2048xui16> loc(#loc34)
    %15 = stablehlo.bitcast_convert %14 : (tensor<16x512x2048xui16>) -> tensor<16x512x2048xbf16> loc(#loc35)
    %cst = stablehlo.constant dense<1.000000e+00> : tensor<bf16> loc(#loc23)
    %16 = stablehlo.broadcast_in_dim %cst, dims = [] : (tensor<bf16>) -> tensor<16x512x2048xbf16> loc(#loc36)
    %17 = stablehlo.subtract %15, %16 : tensor<16x512x2048xbf16> loc(#loc36)
    ...
    %20 = stablehlo.broadcast_in_dim %19, dims = [0, 1, 2] : (tensor<1x1x1xbf16>) -> tensor<16x512x2048xbf16> loc(#loc37)
    %21 = stablehlo.multiply %17, %20 : tensor<16x512x2048xbf16> loc(#loc37)
    %22 = stablehlo.broadcast_in_dim %0, dims = [1, 2] : (tensor<1x1xbf16>) -> tensor<1x1x1xbf16> loc(#loc26)
    %23 = stablehlo.broadcast_in_dim %22, dims = [0, 1, 2] : (tensor<1x1x1xbf16>) -> tensor<16x512x2048xbf16> loc(#loc38)
    %24 = stablehlo.add %21, %23 : tensor<16x512x2048xbf16> loc(#loc38)
    %25 = stablehlo.broadcast_in_dim %0, dims = [1, 2] : (tensor<1x1xbf16>) -> tensor<1x1x1xbf16> loc(#loc26)
    %26 = stablehlo.broadcast_in_dim %25, dims = [0, 1, 2] : (tensor<1x1x1xbf16>) -> tensor<16x512x2048xbf16> loc(#loc39)
    %27 = stablehlo.maximum %26, %24 : tensor<16x512x2048xbf16> loc(#loc39)
    return %27 : tensor<16x512x2048xbf16> loc(#loc23)
  } loc(#loc23)
} loc(#loc)
