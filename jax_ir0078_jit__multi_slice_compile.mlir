...
module @jit__multi_slice attributes {mhlo.num_partitions = 1 : i32, mhlo.num_replicas = 1 : i32} {
  func.func public @main(%arg0: tensor<2048x2048xbf16> loc("self")) -> (tensor<1024x2048xbf16> {jax.result_info = "[0]"}, tensor<1024x2048xbf16> {jax.result_info = "[1]"}) {
    %0 = stablehlo.slice %arg0 [0:1024, 0:2048] : (tensor<2048x2048xbf16>) -> tensor<1024x2048xbf16> loc(#loc16)
    %1 = stablehlo.slice %arg0 [1024:2048, 0:2048] : (tensor<2048x2048xbf16>) -> tensor<1024x2048xbf16> loc(#loc16)
    return %0, %1 : tensor<1024x2048xbf16>, tensor<1024x2048xbf16> loc(#loc)
  } loc(#loc)
} loc(#loc)
#loc = loc(unknown)
...
#loc4 = loc("/home/ubuntu/roadrunner-fork/src/felafax/trainer_engine/checkpoint.py":287:0)
